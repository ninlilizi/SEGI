...
public sealed class SEGICascaded : PostProcessEffectSettings
{
    ...
    public VoxelResolution voxelResolution = new ParameterOverride<VoxelResolution> { value = VoxelResolution.high };
    ...
    public ParameterOverride Sun = new ParameterOverride<Light>();
}
...
public sealed class SEGIRenderer : PostProcessEffectRenderer<SEGICascaded>
{
    ...
    int dummyVoxelResolution
    {
        get
        {
            return (int)settings.voxelResolution * (settings.voxelAA ? 2 : 1);
        }
    }
    ...
    float voxelScaleFactor
    {
        get
        {
            return (float)settings.voxelResolution / 256.0f;
        }
    }
    ...
    public override void Render(PostProcessRenderContext context)
    {
        ...
        if (volumeTextures[0].width != (int)settings.voxelResolution)
        {
        ...
        Shader.SetGlobalInt("SEGIVoxelResolution", (int)settings.voxelResolution);
        ...
        Shader.SetGlobalFloat("SEGISecondaryBounceGain", settings.infiniteBounces ? secondaryBounceGain : 0.0f);
        ...
        if (renderState == RenderState.Voxelize && updateVoxelsAfterXDoUpdate == true)
        {
            ...
            float voxelTexel = (1.0f * voxelSpaceSize) / (int)settings.voxelResolution * 0.5f;
            ...
            {
                origin = attachedCamera.transform.position + attachedCamera.transform.forward * voxelSpaceSize / 4.0f;
            }
            ...
            clearCompute.SetInt("Res", (int)settings.voxelResolution);
            clearCompute.Dispatch(0, (int)settings.voxelResolution / 16, (int)settings.voxelResolution / 16, 1);
            ...
            transferInts.SetInt("Resolution", (int)settings.voxelResolution);
            transferInts.SetVector("VoxelOriginDelta", (voxelSpaceOriginDelta / voxelSpaceSize) * (int)settings.voxelResolution);
            transferInts.Dispatch(0, (int)settings.voxelResolution / 16, (int)settings.voxelResolution / 16, 1);
            ...
            for (int i = 0; i < numMipLevels - 1; i++)
            {
                ...
                int destinationRes = (int)settings.voxelResolution / Mathf.RoundToInt(Mathf.Pow((float)2, (float)i + 1.0f));
                ...
                mipFilter.Dispatch(mipFilterKernel, (int)settings.voxelResolution / 16, (int)settings.voxelResolution / 16, 1);
        ...
        else if (renderState == RenderState.Bounce && updateVoxelsAfterXDoUpdate == true)
        {
            ...
            transferInts.SetInt("Resolution", (int)settings.voxelResolution);
            transferInts.Dispatch(1, (int)settings.voxelResolution / 16, (int)settings.voxelResolution / 16, 1);
    ...
    void CreateVolumeTextures()
    {
        ...
        for (int i = 0; i < numMipLevels; i++)
        {
            ...
            int resolution = (int)settings.voxelResolution / Mathf.RoundToInt(Mathf.Pow((float)2, (float)i));
        ...
        volumeTextureB = new RenderTexture((int)settings.voxelResolution, (int)settings.voxelResolution, 0, RenderTextureFormat.ARGBHalf, RenderTextureReadWrite.Default);
        volumeTextureB.dimension = TextureDimension.Tex3D;
        volumeTextureB.volumeDepth = (int)settings.voxelResolution;
        ...
        volumeTexture1 = new RenderTexture((int)settings.voxelResolution, (int)settings.voxelResolution, 0, RenderTextureFormat.ARGBHalf, RenderTextureReadWrite.Default);
        volumeTexture1.dimension = TextureDimension.Tex3D;
        volumeTexture1.volumeDepth = (int)settings.voxelResolution;
        ...
        intTex1 = new RenderTexture((int)settings.voxelResolution, (int)settings.voxelResolution, 0, RenderTextureFormat.RInt, RenderTextureReadWrite.Default);
        intTex1.dimension = TextureDimension.Tex3D;
        intTex1.volumeDepth = (int)settings.voxelResolution;
    ...
    void ResizeDummyTexture()
    {
        ...
        dummyVoxelTexture2 = new RenderTexture((int)settings.voxelResolution, (int)settings.voxelResolution, 0, RenderTextureFormat.R8);
